...
class PlayerTriggerEventEvent extends CsgoEvent implements \JsonSerializable
{
	private const PATTERN = "(\d{1,2}\/\d{1,2}\/\d{1,4})\s-\s(\d{1,2}:\d{1,2}:\d{1,2}):\s\"(.*?)<(\d{1,6})><(STEAM_[01]:[01]:\d*?)><([A-Za-z]*)>\"\striggered\s\"(.*?)\"";
